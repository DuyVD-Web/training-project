<x-app-layout>
    ...
    <div class="grid grid-cols-4 mt-6 gap-6 py-10">
        <form action="{{route('admin.users.create')}}" class="col-start-2 col-end-4 relative" id="info-form"
              method="POST">
            @csrf
                    ...
                    <div class="w-[45%]">
                        ...
                        <select id="role_id" name="role_id"
                            ...
                            @foreach((new \ReflectionClass(\App\Enums\UserRole::class))->getConstants() as $key => $value)
                                <option value="{{ \App\Models\Role::where('name', $value)->value('id') }}">{{ $key }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
